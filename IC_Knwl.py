...
def text_vectorization(data):
    ...
    model = SentenceTransformer('ml-distiluse-base-multilingual-cased-v2') #  512
    colnames = ['outlet','title_ml','title_en','lang','kg_en','kg_ml','bias']
    df = pandas.read_csv('data/' + data + '.csv', names=colnames, sep='\t')

    lst_summary = df.title_ml.tolist()
    
    #Vectorization of summaries..
    embeddings_summary = model.encode(lst_summary) 
...
def train_save_model(model_name):
    ...
    gate_model = Dense(3, activation='softmax')(encode_summary)
    ...
    if os.path.isfile('models/'+ model_name +'/MODEL.h5') == False:
        ...
        trainque_feature = np.load('embeddings/ml-distiluse-base-multilingual-cased-v2/train_summary.npy')
        validque_feature = np.load('embeddings/ml-distiluse-base-multilingual-cased-v2/valid_summary.npy')
        testque_feature = np.load('embeddings/ml-distiluse-base-multilingual-cased-v2/test_summary.npy')


        history = gate_model.fit([trainque_feature], trainans, epochs=50, batch_size=1024, validation_data=([validque_feature], validans), callbacks=callbacks_list, verbose=1)
...
def main():
     ...
     try:
         ...
         if os.path.isfile('tmp/test_summary.npy') == False:
    ...
    if os.path.exists('models/model-en/') == False:
        os.mkdir('models/model-en/')
        train_save_model("model-en")
        evaluate("model-en")
        shutil.rmtree('models/model-en/', ignore_errors=True)
